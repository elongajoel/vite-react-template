      ...
      <div className="max-w-6xl mx-auto px-2">
        <div className="flex gap-8 overflow-x-auto pb-4">
          {colonnes.map((colonne) => (
            <div
              key={colonne.id}
              className="bg-neutral-50 rounded-2xl shadow-lg p-0 min-w-[340px] flex flex-col border border-neutral-100"
            >
              {/* En-tête coloré Notion */}
              <div className={`flex items-center justify-between px-5 py-4 rounded-t-2xl border-b border-neutral-200 ${colonne.couleur}`}>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-base tracking-wide">{colonne.titre}</span>
                  <span className="ml-2 bg-white/70 rounded-full px-2 py-0.5 text-xs font-semibold border border-white/50">{colonne.cartes.length}</span>
                </div>
                <button
              ...
              <div className="flex flex-col gap-4 px-4 py-4">
                {colonne.cartes.map((carte) => (
                  <div
                    key={carte.id}
                    className="bg-white border border-neutral-100 rounded-xl px-4 py-3 text-neutral-800 shadow-sm flex flex-col gap-2 hover:shadow-md hover:border-blue-200 transition-all group"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-sm truncate pr-2">{carte.titre}</span>
                      <button
                    ...
                    <div className="flex items-center gap-2 text-xs text-neutral-500 mt-1">
                      <span className="inline-flex items-center gap-1">
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 16 16"><circle cx="8" cy="8" r="8"/></svg>
                        {carte.auteur}
                      </span>
                      <span className="px-2 py-0.5 rounded bg-neutral-100 border border-neutral-200 text-neutral-600 font-semibold ml-2">{carte.tag}</span>
                      <span className="ml-auto">{carte.date}</span>
                    </div>
                  </div>
                ))}
              </div>
